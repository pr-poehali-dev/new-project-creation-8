...
function StatCard({ target, display, label, index, active }: { target: number; display: string; label: string; index: number; active: boolean }) {
  ...
  return (
    ...
    >
      <div className="gradient-gold heading-display text-5xl md:text-6xl mb-3 tabular-nums">{shown}</div>
...
export default function WhyImportant() {
  ...
  return (
    <section ref={ref} className="section-dark py-24 px-6">
      <div className="bg-watermark hidden md:block">20</div>
      <div className="max-w-7xl mx-auto relative z-10">
        <p className="section-label mb-3">03 — Почему это важно</p>
        <div className="w-16 h-0.5 bg-brand-gold/40 mb-10" />

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start mb-14">
          <h2 className={`heading-display text-white text-4xl md:text-5xl ${inView ? "animate-fade-up" : "opacity-0"}`}>
            Почему это важно
          </h2>
